...
6. **循环存储**：当 `RCLONE_MAX_SIZE` 大于 0 时，脚本会在每次上传前后检查整个网盘的实际使用量（通过 `rclone about`，包含回收站和其他文件占用），超限时自动按日期删除最早的录像文件，并永久删除（跳过回收站）。例如 Google Drive 免费 15GB 空间，建议设为 `10`，预留 5GB 缓冲
